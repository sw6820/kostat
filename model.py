...
class Model:
    ...
    Get custom_model from only ./model_loc
    """
    ...
    def get_model(self):
        try:
            ...
            self.logger.debug(f"Success Loading Model: {self.model_name}")
        except:
    ...
    def get_tokenizer(self):
        try:
            if self.model_name == "monologg/distilkobert":
                tokenizer = KoBertTokenizer.from_pretrained('monologg/kobert') 

            else:
                tokenizer = AutoTokenizer.from_pretrained(self.model_name)
            self.logger.debug(f"Success Loading tokenizer: {self.model_name}")
        except:
